...
from densefuse_net_test import DenseFuseNet
...
def _handler(ir_path, vis_path, model_path, model_pre_path, ssim_weight, index, output_path=None):
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		save_images(ir_path, result, output_path,
...
def _handler_l1(ir_path, vis_path, model_path, model_pre_path, ssim_weight, index, output_path=None):
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		target = tf.placeholder(
		    tf.float32, shape=temp_out.shape, name='target')
		...
		enc_ir1, enc_ir2, enc_ir3, enc_vis1, enc_vis2, enc_vis3 = sess.run([enc_ir1, enc_ir2, enc_ir3,enc_vis1, enc_vis2, enc_vis3],
		...
		result = np.dstack([output, Cr, Cb])
		result = cv2.cvtColor(result, cv2.COLOR_YCrCb2BGR)
		cv2.imwrite((output_path + str(index) + '_multiScale_densefuse_l1_MRSPECT_ssim' + str(ssim_weight) + '.jpg'), result)
